...
namespace VirtualFileSystem
{
    ...
    [Serializable]
    public class VirtualDirectory : DirectoryBase
    {
        ...
        {
            path = EnsurePathEndsWithDirectorySeparator(VirtualFileDataAccessor.Path.GetFullPath(path));
            var affectedPaths = VirtualFileDataAccessor
                .AllPaths
            ...
            foreach (var affectedPath in affectedPaths)
                VirtualFileDataAccessor.RemoveFile(affectedPath);
        }
        ...
        public override bool Exists(string path)
        {
            try
            {
                ...
                path = VirtualFileDataAccessor.Path.GetFullPath(path);
                return VirtualFileDataAccessor.AllDirectories.Any(p => p.Equals(path, StringComparison.OrdinalIgnoreCase));
            }
            catch (Exception)
            {
                return false;
        ...
        public override string[] GetFiles(string path, string searchPattern, SearchOption searchOption)
        {
            ...
            return GetFilesInternal(VirtualFileDataAccessor.AllFiles, path, searchPattern, searchOption);
        }
        ...
        public override string[] GetLogicalDrives()
        {
            return VirtualFileDataAccessor
                .AllDirectories
                .Select(d => new VirtualDirectoryInfo(VirtualFileDataAccessor, d).Root.FullName)
        ...
        {
            var fullSourcePath = EnsurePathEndsWithDirectorySeparator(VirtualFileDataAccessor.Path.GetFullPath(sourceDirName));
            var fullDestPath = EnsurePathEndsWithDirectorySeparator(VirtualFileDataAccessor.Path.GetFullPath(destDirName));
            ...
            var sourceRoot = VirtualFileDataAccessor.Path.GetPathRoot(fullSourcePath);
            var destinationRoot = VirtualFileDataAccessor.Path.GetPathRoot(fullDestPath);
            ...
            VirtualFileDataAccessor.Directory.CreateDirectory(fullDestPath);

            //Copy over the attributes
            var sourceDirectoryInfo = VirtualFileDataAccessor.DirectoryInfo.FromDirectoryName(sourceDirName);
            var destDirectoryInfo = VirtualFileDataAccessor.DirectoryInfo.FromDirectoryName(destDirName);
            ...
            foreach (var file in files)
            {
                ...
                VirtualFileDataAccessor.FileInfo.FromFileName(file).MoveTo(newFilePath);
            }
        ...
        public override IEnumerable<string> EnumerateDirectories(string path, string searchPattern, SearchOption searchOption)
        {
            ...
            var dirs = GetFilesInternal(VirtualFileDataAccessor.AllDirectories, path, searchPattern, searchOption);
